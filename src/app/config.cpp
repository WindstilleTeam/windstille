  ...
  for(auto const& opt : argp.parse_args(argc, argv))
  {
    switch (opt.key)
    {
      case 'A':
        ...
        {
          throw std::invalid_argument("Anti-Aliasing option '-a' requires argument of type {NUM}");
        }
        else
        {
      ...
      case 'g':
        {
          ...
          {
            throw std::invalid_argument("Geometry option '-g' requires argument of type {WIDTH}x{HEIGHT}");
          }
        }
        break;
      ...
      case 'a':
        {
          ...
          {
            throw std::invalid_argument("Geometry option '-a' requires argument of type {WIDTH}x{HEIGHT}");
          }
        }
        break;
...
Config::is_set(std::string const& name)
{
  ...
  {
    throw std::invalid_argument("Error: unknown Config value: '" + name + "'");
  }
  else
  {
