...
EOF
  ...
  # Optional: libxmp for module music under external/libxmp (Windstille uses ModPlug on desktop/wasm).
  MOD_XMP=false
  if [ -n "${LIBXMP_SRC:-}" ] && [ -d "$LIBXMP_SRC" ]; then
    ...
    # CORE_PLAYER is enough for Windstille .mod/.xm. Only compile the core
