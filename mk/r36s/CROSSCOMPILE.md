# Cross-compiling Windstille for R36S (ArkOS)
...
| Setting | Value |
|---------|--------|
| Video | **SDL2** + **OpenGL ES 2** (`-DWINDSTILLE_USE_GLES=ON`) — Mali-G31 / libmali or Mesa |
| Sound | wstsound when available (`-DWINDSTILLE_ENABLE_SOUND=ON`) |
| XDG | often off on CFW (`-DWINDSTILLE_NO_XDGCPP=ON`) |
...
cmake -S . -B build-r36s \
  ...
  -DWINDSTILLE_USE_GLES=ON -DWINDSTILLE_ENABLE_SOUND=ON -DWINDSTILLE_NO_XDGCPP=ON
...
nix build .#windstille-r36s            # aarch64 binary + data
nix build .#windstille-r36s-portmaster # PortMaster tree under result/
nix build .#windstille-r36s-portmaster-zip
```
...
`WINDSTILLE_USE_GLES`, `WINDSTILLE_ENABLE_SOUND`, and `WINDSTILLE_NO_XDGCPP` to ON; override
...
```bash
nix build .#windstille-r36s-portmaster
cp -a result/"Windstille.sh" result/windstille /roms/ports/

# Or autoinstall zip:
nix build .#windstille-r36s-portmaster-zip
# copy result/windstille.zip → ports/PortMaster/autoinstall/
```
...
```text
/roms/ports/
  Windstille.sh      # launcher (may source PortMaster control.txt)
  windstille/
    windstille       # binary
    data/        # game data
```
...
Windstille uses the **SDL Joystick API** (not GameController). Face buttons and
axes are the kernel’s raw indices; `SDL_GAMECONTROLLERCONFIG` does **not**
remap them inside Windstille.
...
the launch script, not by Windstille itself.
