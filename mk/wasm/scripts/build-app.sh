#!/usr/bin/env bash
# Cross-compiles Windstille to wasm32 + HTML via emcmake.
# Expects (set by nix/wasm.nix):
#   APP_NAME         - binary/html basename (e.g. windstille)
...
#   PROJECT_VERSION_FULL - e.g. 0.1.5-dev+gabc1234 (CMake WINDSTILLE_VERSION)
...
LINK_FLAGS=(
  ...
  # Levelset loading uses try/catch around WindstilleLevel::load (multi-line
...
cmake_args=(
  ...
  -DBUILD_TESTS=OFF -DBUILD_EDITOR=OFF -DBUILD_EXTRA=OFF
  -DBUILD_EXTRA=OFF
  -DWARNINGS=OFF
  -DWERROR=OFF
  -DWINDSTILLE_USE_GLES="$( [ "$ENABLE_GLES2" = 1 ] && echo ON || echo OFF )"
  -DWINDSTILLE_EMSCRIPTEN=ON
  -DPRIO_USE_JSONCPP=OFF
  -DWINDSTILLE_ENABLE_SOUND="$( [ "$ENABLE_SOUND" = 1 ] && echo ON || echo OFF )"
  -DDATA_PREFIX="/data"
...
# Emscripten names the outputs after the CMake target (windstille).
