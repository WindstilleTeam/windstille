...
# Env: APP_NAME (default: windstille), optional WINDSTILLE_WASM_PORT (default 8765; was SUPERTUX_WASM_PORT)
#      WINDSTILLE_WASM_OPEN_QUERY — extra query string (e.g. verbose=1&debug=1)
# Run from the package directory or set PKG to that path.
set -euo pipefail

# CLI flags (from: nix run .#windstille-wasm -- --debug --verbose)
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --help|-h)
      ...
      echo "  --debug    open with ?debug=1  (Windstille -D / developer-mode)"
      echo "  --verbose  open with ?verbose=1 (Windstille -v)"
      echo "Env: APP_NAME, PKG, WINDSTILLE_WASM_PORT, WINDSTILLE_WASM_OPEN_QUERY, BROWSER"
      exit 0
      ;;
    --) shift; break ;;
    -*)
...
app_name="${APP_NAME:-windstille}"
port="${WINDSTILLE_WASM_PORT:-8765}"
...
try:
    ...
    sys.stderr.write(
        ...
        "       set WINDSTILLE_WASM_PORT to a free port\n" % (port, e))
    sys.exit(1)
...
# ignored Cache-Control. Optional WINDSTILLE_WASM_OPEN_QUERY and --debug/--verbose
# append flags (shell.html maps ?debug=1 → -D, ?verbose=1 → -v).
bust=$(date +%s)
parts=("v=${bust}")
extra="${WINDSTILLE_WASM_OPEN_QUERY:-}"
if [ -n "$extra" ]; then
  parts+=("$extra")
fi
